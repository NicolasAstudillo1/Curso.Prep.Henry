...
function dePalabrasAFrase(palabras) {
  ...
  for (let i=0; i < palabras.length-1; i++)
    {stringCompleta = stringCompleta + palabras[i]+ " "}
    return stringCompleta + palabras[palabras.length-1];
}
...
function mayorACien(array) {
  ...
  // Tu código:
 let arrayMayoresCien = [];
for (let i = 0; i < array.length; i++) {
  if (array[i]>100){
    arrayMayoresCien.push(array[i]);
  }
}
  return arrayMayoresCien;
}
